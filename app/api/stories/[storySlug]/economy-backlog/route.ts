...
function isCreatorEconomyBacklogEnabled(): boolean {
  return (
...
export async function GET(
  ...
  try {
    if (!isCreatorEconomyBacklogEnabled()) {
      return apiError({
        status: 404,
        error: "Creator economy backlog is disabled",
        requestId,
      });
    }
    ...
    const outcomeAgentPlan = isFeatureEnabled("economy_outcome_agent")
      ? buildCreatorEconomyOutcomeAgentPlan({
          history: resolved.historyRows,
          currentMetrics: operatingPlan.baselineMetrics,
          learning: policyLearning,
        })
      : null;
    ...
    return apiJson(
      {
        ...
        roleBoard,
        history: resolved.historyRows
          .slice(0, parsedQuery.data.limit ?? 10)
    ...
    return apiInternalError({
      routeName: "api/stories/[storySlug]/economy-backlog:GET",
      requestId,
      error,
      message: "Failed to load creator economy backlog",
    });
  }
}
...
export async function POST(
  ...
  try {
    ...
    const createdRun = parsedBody.data.persist
      ? await createStoryCreatorEconomyRun({
          ...
          plan: {
            ...
            executedRecommendationId: recommendation.id,
            recommendation,
            policyMode: parsedBody.data.mode,
            operatingPlan,
            roleBoard,
            merchPlan,
          },
    ...
    return apiInternalError({
      routeName: "api/stories/[storySlug]/economy-backlog:POST",
      requestId,
      error,
      message: "Failed to execute creator economy backlog item",
    });
  }
}
